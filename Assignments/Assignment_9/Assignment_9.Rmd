...
```
## Tasks to be completed in this Assignment:
...
### Modeling the Predictors of Graduate School Admissions
...
Predictions were created in order to estimate the probability of students that will be admitted to graduate school based on their GPA scores, GRE scores, and their school ranking. A generalized linear model was used to assess the variables. 
```{r}
#adding some predictions (the predictions [pred] represent the probabilty of the students that will make it into grad school based on the variables gpa, rank, and gre)
gradDataPredictions <- add_predictions(gradData, mod1, type="response")
...
##### MODEL 1.1) Graduate School Admissions based on GPA
...
##### MODEL 1.2) Graduate School Admissions based on GRE
```{r}
...
#adding some predictions (the predictions [pred] represent the probabilty of the students that will make it into grad school based on the variables gpa, rank, and gre)
gradDataPredictions <- add_predictions(gradData, mod2, type="response")
...
##### MODEL 2.1) Graduate School Admissions based on GPA
...
##### MODEL 2.2) Graduate School Admissions based on GRE
```{r}
#plotting the predictions based on GRE scores
...
*We will find the mean, in order to find the best fit line model*

<li>The mean of model 1:</li>
```{r}
...
*Looking at the importance of* **GPA scores** *in admissions for the best model:*
```{r}
...
*Looking at the importance of* **GRE scores** *in admissions for the best model:*
```{r}
...
```
<p>These graphs also support the earlier graphs of each model, in which there was significant importance of the high school ranking and high GRE score in the likelihood of those applicants to be admitted into graduate school.
